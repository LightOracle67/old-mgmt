...
<h3><a href = 'invoicesearch.php'><i class = 'bi bi-arrow-left-circle'></i></a><?php echo $detailinvoiceinfo;
?></h3>
<div class = 'table-responsive bg-light'>
<h5><?php echo $username.': '.mysqli_fetch_array( mysqli_query( $con, 'SELECT realname from users where userid = (SELECT distinct(userid) from detailinvoice where invoiceid = '.$invoicedetail.');' ) )[ 0 ];
?></h5>
<h5><?php echo $invoiceid.': '.$invoicedetail;
?></h5>
<div class = 'table-responsive bg-light'>
...
if ( !empty( $row ) ) {
    foreach ( $row as $rows )
 {
    ...
    <th><?php echo $prodname;
    ?></th>
    <th><?php echo $prodprice;
    ?></th>
    <th><?php echo $quantity;
    ?></th>
    <th><?php echo $checkout;
    ?></th>
    <th><?php echo $checkoutiva;
    ?></th>
    </tr>
    </thead>
    <tbody class = 'table-group-divider'>
        ...
        foreach ( $row2 as $rows2 )
 {
            ...
            <td><?php echo $rows2[ 'prodid' ];
            ?></td>
            <td><?php echo mysqli_fetch_array( mysqli_query( $con, 'SELECT prodname from products where prodid = '.$rows2[ 'prodid' ].';' ) )[ 0 ];
            ?></td>
            <td><?php echo $rows2[ 'price' ].$currencies ?></td>
            <td><?php echo $rows2[ 'quantity' ];
            ?></td>
            <td><?php echo $rows2[ 'checkout' ];
            ?></td>
            <td><?php echo $rows2[ 'checkoutplusiva' ];
            ?></td>
            </tr>
            <?php }
            ;
        ...
        <caption class = 'sticky-bottom'><?php echo( '('.mysqli_fetch_array( mysqli_query( $con, 'SELECT count(*) FROM detailinvoice where invoiceid = '.$invoicedetail.';' ) )[ 0 ].$details2show );
        ?></caption>
        </div></div>
        </div>
        </body>
