...
if (!isset($_POST['add']) && !isset($_POST['delete']) && !isset($_POST['edit'])) {
    ...
    if (empty($_FILES['newprodimage[]'])) {
        $newprodimage = mysqli_fetch_array(mysqli_query($con, 'SELECT image from products WHERE prodid = ' . $selprodbyid . ';'))[0];
    } else {
        foreach ($_FILES['newprodimage']['error'] as $key => $error) {
            if ($error == UPLOAD_ERR_OK) {
                $tmpName = $_FILES['newprodimage']['tmp_name'][$key];
                $newprodimage = basename($_FILES['newprodimage']['name'][$key]);
                move_uploaded_file($tmpName, "./prodimages/$newprodimage");
            }
        $newprodimage = $prodimage;
    };
    editprod($selprodbyid, $newrealid, $newprodname, $newfullname, $newproddesc, $newdateadded, $newprice, $newclass, $newtype, $newprodimage);
    exit();
    };
};
