...
if ( mysqli_connect_errno() ) {
    exit ("<div class='alert alert-danger' role='alert'>".$dberror."</div>");
    };
    ...
    echo ("<option selected>".$selectusername."</option>");
    ...
    for($x=0;$x<mysqli_num_rows(mysqli_query($con,"SELECT userid FROM store.users;"));$x++){
        ...
        echo ("</option>");
            ...
            <?php
$realpassword = hash("sha512",$_POST["password"]);
if ( mysqli_connect_errno() ) {
...
if ($stmt = $con->prepare('SELECT userid,password FROM users WHERE username = ?')) {
    ...
    if ($stmt->num_rows > 0) {
        ...
        if ($realpassword === $password){ 
            ...
            echo ("<div class='alert alert-danger' role='alert'>".$usrpasserror."</div>");
        }
    } else {
        echo ("<div class='alert alert-danger' role='alert'>".$usrpasserror."</div>");
    }
